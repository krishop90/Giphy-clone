...
export const FilterGifs = ({alignLeft = false , showTrending=false}) => {
    ...
    return (
    <div className={`flex m-3 gap-3 ${alignLeft ? "" : "justify-end"} ${showTrending ? "justify-between flex-col sm:flex-row sm:items-center" : ""}`}>
        {
            showTrending && (
                <span className='flex gap-2'>
                {
                    showTrending && (
                        <HiOutlineTrendingUp size={25} className="text-teal-400" />
                    )
                }
